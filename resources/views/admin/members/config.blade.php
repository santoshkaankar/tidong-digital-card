...
<head>
    ...
    <title>Admin - Master Profile Configuration - Tidong® Portal</title>
...
<body>
    ...
    <div id="content">
        ...
        <nav class="top-navbar">
            ...
            <div class="navbar-brand fw-bold text-dark mb-0 h6 d-flex align-items-center gap-2">
                <i class="fas fa-user-shield text-primary"></i> Admin Panel: Master Profile & Card Configuration
            </div>
            <div class="ms-auto d-flex gap-2">
                <a href="{{ route('admin.members.manage') }}" class="btn btn-outline-dark btn-sm">
                    <i class="fas fa-arrow-left me-1"></i> Reset Search
                </a>
            </div>
        </nav>
                ...
                <div class="col-lg-11">
                    ...
                    @if(isset($member))
                        <div class="form-card p-4 p-md-5">
                            ...
                            <!-- Header Banner -->
                            <div class="bg-dark text-white p-3 rounded-3 mb-4 d-flex justify-content-between align-items-center flex-wrap gap-2">
                                <div class="fw-bold fs-5">
                                    <i class="fas fa-sliders-h text-info me-2"></i> Admin Mode: Managing Details for {{ $member->name }}
                                </div>
                                <div>
                                    @if(isset($card) && !empty($card->card_no))
                                        <span class="badge bg-success px-3 py-2 fs-6">
                                            <i class="fas fa-id-card me-1"></i> Card No: {{ $card->card_no }}
                                        </span>
                                    @else
                                        <span class="badge bg-warning text-dark px-3 py-2 fs-6">
                                            <i class="fas fa-clock me-1"></i> Not Issued Yet (Generated Post Card Issue)
                                        </span>
                                    @endif
                                </div>
                            </div>

                            <form action="{{ route('admin.members.update.details', $member->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="row g-3 mb-4">
                                    <div class="col-md-4">
                                        <label class="form-label fw-bold text-secondary small">Full Name / Display Name *</label>
                                        <input type="text" name="name" class="form-control bg-light" required value="{{ old('name', $card->name ?? $member->name) }}">
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label fw-bold text-secondary small">Business Name</label>
                                        <input type="text" name="business_name" class="form-control bg-light" placeholder="e.g. Tidong Marketing Pvt. Ltd." value="{{ old('business_name', $card->business_name ?? '') }}">
                                    </div>
                                    <div class="col-md-4">
                                ...
                                <div class="row g-3 mb-4">
                                    <div class="col-md-4">
                                        ...
                                        <input type="text" name="phone" class="form-control bg-light" required placeholder="98765xxxxx" value="{{ old('phone', $card->phone ?? $member->mobile ?? '') }}">
                                    </div>
                                    <div class="col-md-4">
                                ...
                                <div class="row g-3 mb-4">
                                    <div class="col-md-4">
                                        <label class="form-label text-secondary small">Gmail / Primary Email</label>
                                        <input type="email" name="gmail" class="form-control bg-light" placeholder="example@gmail.com" value="{{ old('gmail', $card->gmail ?? $member->email ?? '') }}">
                                    </div>
                                    <div class="col-md-4">
 ...
 <!-- 7. Address & Location -->
                            ...
                            <div class="row g-3 mb-4">
                                ...
                                <div class="row g-3 mb-4">
                                    <div class="col-md-6">
                                        ...
                                        <textarea name="about_us" class="form-control bg-light" rows="3" placeholder="Brief info about business...">{{ old('about_us', $card->about_us ?? '') }}</textarea>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label text-secondary small">Key Products or Services</label>
                                        <textarea name="services_or_products" class="form-control bg-light" rows="3" placeholder="List key products or services...">{{ old('services_or_products', $card->services_or_products ?? '') }}</textarea>
                                    </div>
                                </div>
                                ...
                                <div class="d-grid mt-4">
                                    <button type="submit" class="btn btn-primary btn-lg py-3 fw-bold shadow-sm">
                                        <i class="fas fa-save me-2"></i> Update Member Details (Admin)
                                    </button>
                                </div>
                    ...
                    @elseif(request('query'))
                        <!-- IF SEARCHED BUT NO MEMBER FOUND -->
                        <div class="alert alert-warning rounded-4 text-center py-4">
                            <i class="fas fa-exclamation-triangle fa-2x mb-2 text-warning"></i>
                            <h5 class="fw-bold">No Member Found</h5>
                            <p class="mb-0">No registered member found matching "<strong>{{ request('query') }}</strong>". Please check the phone number or email ID.</p>
                        </div>
                    @endif
   ...
   $(document).ready(function() {
    // 1. Common Select2 Initializer Function
    function initLocationSelect2(selector, placeholderText) {
        $(selector).select2({
            theme: 'bootstrap-5',
            placeholder: placeholderText,
            allowClear: true,
            minimumInputLength: 2, // 2 characters type karne par AJAX chalega
            ajax: {
                url: "/admin/search-locations",
                dataType: 'json',
                delay: 250,
    ...
    // Teeno fields par Search Enable karna
    initLocationSelect2('#area_search', 'Search Area / Post Office');
    initLocationSelect2('#pincode_search', 'Search Pincode');
    initLocationSelect2('#city_search', 'Search City / District');

    // Select2 Field ko dynamically update karne ke liye Helper Function
    function updateSelect2Field(selector, value, text) {
        if (!value) return;
        let element = $(selector);
        
        // Agar Option pehle se list me nahi h, toh Naya Option Insert karein
    ...
    // 2. Teeno me se KISI KO BHI SELECT karne par charo Auto-Fill honge
    $('#area_search, #pincode_search, #city_search').on('select2:select', function(e) {
        let data = e.params.data;
        if (data) {
            // Fill Area
            if (data.area) {
                updateSelect2Field('#area_search', data.area, data.area);
            }
            // Fill Pincode
            if (data.pincode) {
                updateSelect2Field('#pincode_search', data.pincode, data.pincode);
            }
            // Fill City
            if (data.city) {
                updateSelect2Field('#city_search', data.city, data.city);
            }
            // Fill State
            if (data.state) {
