...
class Chat_model extends CI_Model
{
    ...
    function load_chat($from_id, $to_id, $type = '',  $offset = '', $limit = '', $sort = '', $order = '', $search = '')
    {
        ...
        foreach ($messages as $message) {
            ...
            $product['msg'][$i]['profile'] = base_url() . USER_IMG_PATH . str_replace(' ', '_', $me_user[0]['image'] ?? '');
    ...
    {
        $query = $this->db->query("SELECT * FROM chat_media WHERE message_id=$msg_id ");
        $media_files = $query->result_array();
        
        // Sanitize file paths - replace spaces with underscores
        foreach ($media_files as &$media) {
            if (isset($media['sub_directory'])) {
                $media['sub_directory'] = str_replace(' ', '_', $media['sub_directory']);
            }
            if (isset($media['file_name'])) {
                $media['file_name'] = str_replace(' ', '_', $media['file_name']);
            }
        }
