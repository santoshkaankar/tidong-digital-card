...
class Invoice_model extends CI_Model
{
    ...
    public function get_seller_sales_list(
        ...
        if (!empty($_GET['start_date']) && !empty($_GET['end_date']) || !empty($_POST['start_date']) && !empty($_POST['end_date'])) {
            ...
            $count_res->where(" DATE(o.date_added) <= DATE('" . $date . "') ");
        }
        if (isset($filters) && !empty($filters)) {
            $this->db->group_Start();
            $count_res->or_like($filters);
        ...
        $search_res = $this->db->select(' o.*,oitems.* , u.username ,u.email,u.mobile,u.image as user_profile , sd.store_name,us.username as seller_name ')
            ->join('users u', 'u.id= o.user_id', 'left')
            ->join('order_items oitems', 'o.id=oitems.order_id', 'left')
            ->join('seller_data sd', 'sd.user_id=oitems.seller_id', 'left')
            ->join('users us ', ' us.id = oitems.seller_id', 'left');
        if (!empty($_GET['seller_id']) || !empty($_POST['seller_id'])) {
            
            $seller_id = (!empty($_GET['seller_id']) && isset($_GET['seller_id'])) ? $_GET['seller_id'] : $_POST['seller_id'];
            $search_res->where("oitems.seller_id=" . $seller_id);
        ...
        if (!empty($_GET['start_date']) && !empty($_GET['end_date']) || !empty($_POST['start_date']) && !empty($_POST['end_date'])) {
            ...
            $search_res->where(" DATE(o.date_added) <= DATE('" . $date . "') ");
        }
        if (isset($filters) && !empty($filters)) {
            $search_res->group_Start();
            $search_res->or_like($filters);
